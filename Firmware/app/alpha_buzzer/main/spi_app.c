...
**********************************************************************************/
void spi_to_noise_click_transmit_cmd(spi_device_handle_t spi){

    // creating the CW + Reference Voltage
    uint16_t data = ((COMMAND_SPI & 0xF) << 12) | (V_IN_NOISE_CLICK & 0xFFF);
    ...
    int V_out = ((V_REF_DAC * V_IN_NOISE_CLICK * GAIN_V_REF) / 4096) * 1000;
