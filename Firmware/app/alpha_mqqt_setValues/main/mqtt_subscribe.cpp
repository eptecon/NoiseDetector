	...
	if (mqtt_recMsg != NULL){
        ...
        switch(active_command.method) {
            case METHOD_SET_VOLTAGE:
                printf("Command was to set voltage to %i\n", active_command.voltage);
                vTaskDelay(10 / portTICK_RATE_MS);
                new_voltage = active_command.voltage * 3000 / 100;
                spi_to_noise_click_transmit_cmd(spi, new_voltage);
            ...
            case METHOD_SET_THRESHOLD:
                ...
                vTaskDelay(10 / portTICK_RATE_MS);
                noise_threshold = active_command.threshold; // change the global threshold
                mqtt_update_json_threshold(noise_threshold); // BATTERY TEST
                mqtt_update_json_battery(); // BATTERY TEST
                mqtt_publish((char*) TOPIC2, mqtt_msg);
                break;
            case NO_METHOD:
