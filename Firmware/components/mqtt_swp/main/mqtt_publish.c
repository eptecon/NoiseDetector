...
void mqtt_task(void *arg)
{
    ...
    while(1) {
        ...
        */
		...
		clientId.cstring = "ESP32_Test";
		
		MQTTString user = MQTTString_initializer;
		user.cstring= "SZDjEmfGLXuabfAYHFGg"; //ESP32 test on own AWS instance
		...
		char *out;
		char *msgbuf;
		char temp_value[32], temp_value_new[32], hum_value[32], hum_value_new[32];

		sprintf(temp_value, "%d",(uint8_t)(esp_random()&0xFF));
		sprintf(hum_value, "%d",(uint8_t)(esp_random()&0xFF));
		
		/*Create JSON Object*/
		out = json_create(temp_value, hum_value);
		printf("%s\n",out);
		...
		for (int i=0;i<100;i++) {
			...
			sprintf(temp_value_new, "%d",(uint8_t)(esp_random()&0xFF));
			sprintf(hum_value_new, "%d",(uint8_t)(esp_random()&0xFF));
			
			/*Update JSON Object*/
			msgbuf = json_update(out, temp_value_new, hum_value_new);
			//printf("%s\n",msgbuf);
			...
			ret = MQTTPublish(&client, "v1/devices/me/telemetry", &message);
			if (ret != SUCCESS) {
